...
<strong>⚠️ WARNING：persist / merge の誤解</strong>  

`persist` と `merge` は、  
「どちらも保存するメソッド」と思われがちですが、**役割はまったく違います。**

- `persist`
  - **新規エンティティ**を永続化コンテキストに登録する
  - 管理状態（managed）になる
  - 既にDBに存在するエンティティに使うと例外になる

- `merge`
  - **すでに存在する（または存在するかもしれない）エンティティ**の状態をコピーする
  - 戻り値として **新しい managed エンティティ** が返る
  - 引数で渡したエンティティ自体は managed にならない

特に注意が必要なのは `merge` です。

「merge を呼べば渡したエンティティが管理状態になる」  
と誤解すると、  
**変更が反映されない / 意図しない更新が走る**  
といったトラブルにつながります。
